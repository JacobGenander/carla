...
%
    ...
    %% Pre-process the data for usage

    % Compute front wheel angle
    delta_f = deg2rad(state.steer*70);

    
    %% Evaluate network and return output
    % Evaluate network
    input = [state.a_v'; state.forward_speed'; state.forward_speed.^2'; delta_f'];
    output = net(input);
